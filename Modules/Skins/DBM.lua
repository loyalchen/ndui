...
DBMSkin:SetScript("OnEvent", function()
		...
		for bar in self:GetBarIterator() do
			if not bar.injected then
				...
				if not name.styled then
					...
					name:SetWidth(165)
					name:SetHeight(8)
					name:SetFont(STANDARD_TEXT_FONT, 14, "THINOUTLINE")
				...
				if not timer.styled then	
					...
					timer:SetFont(STANDARD_TEXT_FONT, 14, "THINOUTLINE")
		...
		if not anchor.styled then
			local header={anchor:GetRegions()}
				if header[1]:IsObjectType("FontString") then
					header[1]:SetFont(STANDARD_TEXT_FONT, 14, "THINOUTLINE")
	...
	local function SkinRange(self)
		if not self.styled and DBMRangeCheckRadar then
			...
			DBMRangeCheckRadar.text:SetFont(DBMRangeCheckRadar.text:GetFont(), 16, "THINOUTLINE")
			self.styled = true
		end
	end
	...
	DBM_AllSavedOptions["Default"]["HideGarrisonUpdates"] = true
