...
mono:SetPoint("LEFT", WorldMapTitleButton, "RIGHT", -170, -2)
mono:SetSize(26, 26)
B.CreateCB(mono)
mono.text = B.CreateFS(mono, 14, AUTO_COMPLETE, "LEFT", 25, 0, true)
mono.text:SetTextColor(1, .82, 0)
mono:RegisterEvent("PLAYER_LOGIN")
mono:SetScript("OnEvent", function(self)
	if not NDuiADB["Quest"] then NDuiADB["Quest"] = {} end
	self:SetChecked(NDuiADB["Quest"].AutoComplete)
end)
mono:SetScript("OnClick", function(self)
	if self:GetChecked() then
		NDuiADB["Quest"].AutoComplete = true
	else
		NDuiADB["Quest"].AutoComplete = false
	end
end)
...
function QuickQuest:Register(event, func)
	...
	self[event] = function(...)
		if NDuiADB["Quest"].AutoComplete then
