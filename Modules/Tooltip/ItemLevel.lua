...
local upgradeTable = DB.UpgradeTable
local PREFIX = STAT_AVERAGE_ITEM_LEVEL..": "..DB.InfoColor
...
do
	...
	local function scan(unit, slot, total, count, twoHanded, incomplete)
		if ( slot > INVSLOT_LAST_EQUIPPED ) then
			return formatString:format(total / ((twoHanded and count - 2 or count-1) + 0.0001)), incomplete
		end
		...
		if ( link ) then
			repeat
				...
				if level and level >= 458 then
					local upgradeId = tonumber(strmatch(link, ":(%d+):%d:%d|h"))
